...
object ParallelParenthesesBalancing {
  ...
  def parBalance(chars: Array[Char], threshold: Int): Boolean = {
    def traverse(from: Int, until: Int, left: Int, right: Int): (Int, Int) = {
        var index = from
        var (leftAcc: Int, rightAcc: Int) = (left, right)
        while (index < until || index == from) {
          if (chars(index) == '(') leftAcc += 1
          else if (chars(index) == ')') {
            if (leftAcc == 0) rightAcc -= 1
            else leftAcc -= 1
          }
          index += 1
        }
    ...
    def reduce(from: Int, until: Int): (Int, Int) = {
      if (until - from <= threshold) traverse(from, until, 0, 0)
        ...
        if (v1._2 == 0 && v2._1 == 0) {
          if (v1._1 > -v2._2) (v1._1 + v2._2, 0)
          else (0, v2._2 + v1._1)
        }
        else (v1._1 + v2._1, v1._2 + v2._2)
      }
    }
