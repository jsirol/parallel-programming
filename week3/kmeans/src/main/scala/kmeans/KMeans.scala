...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var res = points.groupBy({x => findClosest(x, means)})
    means.filter({x => !res.keySet.contains(x)}).foreach({ p => res += (p -> IndexedSeq()) })
    res
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (0 until oldMeans.length).forall(ix => oldMeans(ix).squareDistance(newMeans(ix)) <= eta)
  }
